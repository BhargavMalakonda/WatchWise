...
async def analyze_video(request: Request, body: AnalyzeRequest) -> AnalyzeResponse:
    ...
    fetch_video_task = loop.run_in_executor(None, fetch_video_data, video_id)
    fetch_transcript_task = loop.run_in_executor(None, fetch_transcript, video_id)

    try:
        video_data, transcript = await asyncio.gather(
            fetch_video_task, fetch_transcript_task
        )
    except YouTubeDataError as exc:
        # str(exc) never contains the API key — safe to surface
